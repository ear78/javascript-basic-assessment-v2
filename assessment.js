...
for(var key in briefing) {
	// var secretVar = [key].substring(0, 6);
	var secretVar = [key].toString();
	if(secretVar.substring(0,6) === 'secret'){
...
function tripleTheFun(p1){
	if(typeof p1 === 'number'){
		var number = p1 * 3;
		return number;
	}
	else if(typeof p1 === 'string'){
		var string = p1 + string;
		return string;
	}
...
function clockIn(employee, department, time){
	var d = new Date;
...
function callerBack(back, holla){
	return holla(back);
}

callerBack(' back', function(){
	return 'Give it' + back;
});
